...
namespace VCX::Labs::Fluid {
        ...
        CaseAPIC::CaseAPIC(std::initializer_list<Assets::ExampleScene> && scenes):
            _scenes(scenes),
            _program(
        ...
        Common::CaseRenderResult CaseAPIC::OnRender(std::pair<std::uint32_t, std::uint32_t> const desiredSize) {
            if (_recompute) {
                _recompute = false;
        ...
        void CaseAPIC::OnProcessInput(ImVec2 const & pos) {
            _cameraManager.ProcessInput(_sceneObject.Camera, pos);
        }

        void CaseAPIC::ResetSystem() {
